...
void main() {
  ...
  test('should start timer for a task via repository', () async {
    // arrange
    ...
    when(mockRepository.startTimer(tTaskId))
        .thenAnswer((_) async => Success(tTimeLog));

    // act
    final result = await usecase(const StartTimerParams(tTaskId));

    // assert
    expect(result, isA<Success<TimeLog>>());
    expect((result as Success<TimeLog>).value, tTimeLog);
    verify(mockRepository.startTimer(tTaskId)).called(1);
