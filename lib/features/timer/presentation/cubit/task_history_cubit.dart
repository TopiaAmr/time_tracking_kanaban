...
class TaskHistoryCubit extends Cubit<TaskHistoryState> {
  ...
  /// Use case for getting detailed completed tasks history.
  final GetCompletedTasksHistoryDetailedUseCase _getCompletedTasksHistoryDetailedUseCase;

  /// Creates a [TaskHistoryCubit] with the required use cases.
  TaskHistoryCubit(
    this._getCompletedTasksHistoryUseCase,
    this._getCompletedTasksHistoryDetailedUseCase,
  ...
  /// Loads the detailed history of completed tasks with time logs.
  Future<void> loadDetailedHistory() async {
    // Only show loading if we don't already have loaded data
    if (state is! TaskHistoryDetailedLoaded) {
      emit(const TaskHistoryLoading());
    }

    final result = await _getCompletedTasksHistoryDetailedUseCase(NoParams());

    if (result is Error<List<TaskHistoryDetail>>) {
      // Only emit error if we don't have existing data to show
      if (state is! TaskHistoryDetailedLoaded) {
        emit(TaskHistoryError(result.failure));
      }
    ...
    final details = (result as Success<List<TaskHistoryDetail>>).value;
    emit(TaskHistoryDetailedLoaded(details));
  }
}
